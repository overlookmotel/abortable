...
class Abortable extends Promise {
		...
		if (value) {
			...
			if (isAbortable(value)) {
				...
				if (err) {
					...
					// TODO pass 3rd arg for indirect?
					value.abort(err, () => this._abortDone());
				}
	...
	abort(err, cb) {
		...
		if (cb == null) {
			...
			throw new Error('.abort() callback must be a function or null');
		}
	...
	_abortPropagate(target) {
		if (isAbortableInstance(target)) {
			target._abortIndirect(this._abortError, () => this._abortDone());
		} else {
			this._awaiting = undefined;
			// TODO pass 3rd arg for indirect?
			target.abort(this._abortError, () => this._abortDone());
		}
	}
