...
class Abortable extends Promise {
	...
	constructor(executor) {
		...
		// Substitute real Abortable for dummy
	 ...
	 * @private
	 * @param {*} res - Value promise resolved with
	 * @returns {undefined}
	 */
	_settled(res) {
		if (isAbortable(res)) {
	 ...
	 * @private
	 * @param {Abortable|DummyAbortable} follower - Promise which is following this one.
	 * @returns {number} - New length of followers array
	 */
	_followed(follower) {
		// Increment unaborted followers/awaiters count
		this._unabortedCount++;

		// Set `_awaited` on follower to this
	...
	static resolve(value) {
		// If res is an instance of Abortable class, return it unchanged
