...
const AbortError = require('./error'),
	...
	CollectionAll = require('./collectionAll'),
	CollectionRace = require('./collectionRace'),
	{isPromise, isAbortable} = require('./utils');
...
class Abortable extends Promise {
	...
	static all(iterator, options) {
		return new CollectionAll(iterator, options).run();
	}

	static race(iterator, options) {
