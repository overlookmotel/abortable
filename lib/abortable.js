...
// Imports
// Imported below class definition to avoid circular references with utils.js
let DummyAbortable, definePrivateProps;
...
class Abortable extends Promise {
	...
	constructor(executor) {
		...
		definePrivateProps(this, {
			_canAbort: p._canAbort,
			_isFateSealed: p._isFateSealed,
			_abortHandler: p._abortHandler,
			_onAbortWasCalled: p._onAbortWasCalled,
			_awaiting: awaiting,
			_followers: undefined,
			_isAborted: false,
			_abortError: undefined,
			_unabortedCount: 0,
			_expectedInternalThenCount: 0
		});
	...
	then(resolveHandler, rejectHandler) {
		this._debug('then', `(_expectedInternalThenCount: ${this._expectedInternalThenCount})`);
		...
		if (this._expectedInternalThenCount > 0) {
			this._expectedInternalThenCount--;
...
definePrivateProps = require('./utils.js').definePrivateProps;
