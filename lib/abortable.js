...
class Abortable extends Promise {
	...
	constructor(executor) {
		...
		// TODO For debugging only - delete this
		// console.log('> constructor:', nextId); // eslint-disable-line no-console
		p._id = nextId++;
		...
		this._followedButNotThennedCount = 0;

		// TODO For debugging only - delete this
		this._id = p._id;

		// If following, record following
		if (awaiting) awaiting._followed(this);
	...
	then(resolveHandler, rejectHandler) {
		...
		// TODO For debugging only - delete this
		// this._log('then created promise', `new promise: ${p._id}`);

		// Increment unaborted followers/awaiters count
		if (this._followedButNotThennedCount > 0) {
			// This `.then()` call due to following, and count already incremented.
			// See comments in `._followed` method for more detail.
			this._followedButNotThennedCount--;
		} else {
	...
	_log(msg, extra) {
		...
		console.log(`> ${msg}: ${this._logId(this)}${extra ? ` ${extra}` : ''}`);
	}

	_logId(p) { // eslint-disable-line class-methods-use-this
		const name = p._name;
		if (!name) return `${p._id}`;
		return `${p._id} ${name}`;
	}
}
