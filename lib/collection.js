...
class Collection {
	...
	followThenable(value, then, index, followDepth) {
		...
		try {
			...
			then.call(
				value,
				res => this.thenResolveCallback(res, index, followDepth, delayByMicrotick),
				err => this.thenRejectCallback(err, index, followDepth, delayByMicrotick)
			);
			if (value instanceof Promise) delayByMicrotick = false;
		} catch (err) {
			this.thenRejectCallback(err, index, followDepth, true);
		}
	}
	 ...
	 * @returns {undefined}
	 */
	thenResolveCallback(value, index, followDepth, delayByMicrotick) {
		if (this.shouldIgnoreThenCallback(index, followDepth)) return;

		const then = this.processItem(value, index, delayByMicrotick);
		if (then) {
	 ...
	 * @returns {undefined}
	 */
	thenRejectCallback(err, index, followDepth, delayByMicrotick) {
		if (this.shouldIgnoreThenCallback(index, followDepth)) return;
		this.resolveOrRejectItem(REJECTED, err, index, delayByMicrotick);
	}
	...
	resolveOrRejectItem(status, value, index, delayByMicrotick) {
		...
		nowOrNextMicrotick(() => this.handleSettleItem(status, value), delayByMicrotick);
	}
