	...
	runDo(iterable) {
		...
		nextMicrotick(() => {
			for (const {then, value, index} of thenables) {
				...
				try {
					let isCalledSync = true;
					then.call(
						value,
						res => this.resolvedOrRejectedOneFromThen(RESOLVED, res, index, isCalledSync),
						err => this.resolvedOrRejectedOneFromThen(REJECTED, err, index, isCalledSync)
					);
					isCalledSync = false;
				} catch (err) {
