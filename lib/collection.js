...
class Collection {
	...
	runDo(iterable) {
		...
		nextMicrotick(() => {
			for (const thenable of thenables) {
				this.followThenable(thenable.value, thenable.then, thenable.index, 0);
			}
	 ...
	 */
	processOne(value, index, delayByMicrotick) { // eslint-disable-line consistent-return
		try {
			...
			this.resolvedOrRejectedOne(RESOLVED, value, index, delayByMicrotick);
		} catch (err) {
			// `.then` getter threw - reject item with thrown error
			this.resolvedOrRejectedOne(REJECTED, err, index, delayByMicrotick);
		}
	}
	 ...
	 * @param {number} index - Input index
	 * @param {number} followDepth - Follow depth
	 * @returns {undefined}
	 */
	followThenable(value, then, index, followDepth) {
		// Call `.then()`.
		...
		try {
			...
			then.call(
				value,
				res => this.resolvedOneFromThen(res, index, followDepth, delayByMicrotick),
				err => this.rejectedOneFromThen(err, index, followDepth, delayByMicrotick)
			);
			if (value instanceof Promise) delayByMicrotick = false;
		} catch (err) {
			this.rejectedOneFromThen(err, index, followDepth, true);
		}
	}

	/**
	 * Thenable called `.then()` resolve callback.
	 * If `.then()` callback called with thenable, follow it. Otherwise, resolve item with value.
	 ...
	 * @returns {undefined}
	 */
	resolvedOneFromThen(value, index, followDepth, delayByMicrotick) {
		if (this.shouldIgnoreThenCallback(index, followDepth)) return;

		const then = this.processOne(value, index, delayByMicrotick);
		if (then) {
			// Increment follow depth, so additional callbacks on previous thenable are ignored
			followDepth++;
			this.state[index].followDepth = followDepth;

			// Call `.then()` in next microtick
			nextMicrotick(() => this.followThenable(value, then, index, followDepth));
		}
	}

	/**
	 * Thenable called `.then()` reject callback.
	 ...
	 * @returns {undefined}
	 */
	rejectedOneFromThen(err, index, followDepth, delayByMicrotick) {
		if (this.shouldIgnoreThenCallback(index, followDepth)) return;
		this.resolvedOrRejectedOne(REJECTED, err, index, delayByMicrotick);
	}

	/**
	 * Check if `.then()` callback should be ingored.
	 * Ignore if already settled (case where resolve/reject callback called more than once).
	 * Ignore if resolve/reject callback called for thenable which already resolved with
	 * another thenable and we're now following that 2nd thenable.
	 * @param {number} index - Input position
	 * @param {number} followDepth - Follow depth
	 * @returns {boolean} - `true` if this callback should be ignored
	 */
	shouldIgnoreThenCallback(index, followDepth) {
		const stateObj = this.state[index];
		return stateObj.status !== PENDING || stateObj.followDepth !== followDepth;
	}
