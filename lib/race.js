...
class CollectionRace extends Collection {
	...
	resolvedOneSimple(value, index) {
		...
		super.resolvedOneSimple(value, index);
	}
	 ...
	 */
		...
		if (this.raceState == RESOLVED) return this.resolve(this.raceResult);
		if (this.raceState == REJECTED) this.reject(this.raceResult);
	}
}
