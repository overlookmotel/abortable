...
module.exports = {
	...
	_followed(follower) {
		// TODO For debugging only - delete this
		this._debug('followed', `(followed by ${this._debugId(follower)})`);
	...
	_followedRecordFollower(follower) {
		// TODO For debugging only - delete this
		this._debug('followedRecord', `(followed by ${this._debugId(follower)})`);
	...
	_debug(msg, extra) {
		...
		console.log(`> ${msg}: ${this._debugId(this)}${extra ? ` ${extra}` : ''}`);
	},
	_debugId(p) {
		const name = p._name;
