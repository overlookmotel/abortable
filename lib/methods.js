...
module.exports = {
	...
	_resolved(resolve, value, ctx) {
		...
		if (DEBUG) {
			Promise.resolve().then(() => {
				this._log('resolved next tick', `(_expectingInternalThen: ${this._expectingInternalThen})`);
			});
		}
	...
	_followed(follower) {
		// TODO For debugging only - delete this
		this._log('followed', `(followed by ${this._logId(follower)})`);
	...
	_followedIncrementCount() {
		// TODO For debugging only - delete this
		this._log('followedIncrementCount');
	...
	_followedRecordFollower(follower) {
		// TODO For debugging only - delete this
		this._log('followedRecord', `(followed by ${this._logId(follower)})`);
