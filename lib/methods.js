/* --------------------
 * abortable module
 * Abortable class prototype methods.
 * To be merged into Abortable class prototype.
 * ------------------*/
...
module.exports = {
	...
	_resolved(resolve, value, ctx) {
		...
		if (this._debugEnabled()) {
			nextMicrotick(() => {
				this._debug(
					'resolved next tick',
					`(_expectedInternalThenCount: ${this._expectedInternalThenCount})`
				);
			});
		}
	...
	_settled(value) {
		...
		if (this._isAborted) {
			// Propogate abortion
			target._abort(this._abortError);
